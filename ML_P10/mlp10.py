...
for i in range(2):
    for j in range(2):
        xA = torch.maximum(boxes1[i, 0], boxes2[j, 0])
        yA = torch.maximum(boxes1[i, 1], boxes2[j, 1])
        xB = torch.minimum(boxes1[i, 2], boxes2[j, 2])
        yB = torch.minimum(boxes1[i, 3], boxes2[j, 3])

        inter = (xB-xA).clamp_min(0.0) * (yB-yA).clamp_min(0.0)
        a1 = (boxes1[i, 2] - boxes1[i, 0]).clamp_min(0.0) * (boxes1[i, 3] - boxes1[i, 1]).clamp_min(0.0)
        a2 = (boxes2[j, 2] - boxes2[j, 0]).clamp_min(0.0) * (boxes2[j, 3] - boxes2[j, 1]).clamp_min(0.0)
        union = a1 + a2 - inter
        iou_ref[i, j] = inter / union.clamp_min(1e-12)

assert torch.allclose(iou, iou_ref, atol=1e-5), f"should be close."
print(f"OK")
