...
def groupby_mean(features: torch.Tensor, group_id: torch.Tensor, K: int, eps: float=1e-12) -> torch.Tensor:
    ...
    out_cnt.index_add_(dim=0, index=group_id, source=ones) #[K], out_cnt[group_id[i]] += ones[i] #[K]
...
group_idx = torch.randint(0, 5, (10,), dtype=torch.long)
out = groupby_mean(features, group_idx, K=5)
...
out_ref = out_ref / cnt_ref.clamp_min(1e-12)[:, None] #[5, 4]
assert torch.allclose(out, out_ref, atol=1e-5), f"should be close."
